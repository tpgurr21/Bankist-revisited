...
const account5 = {
  owner: 'Thomas Gurr',
  movements: [400, -100, 140, 2050, -190],
  interestRate: 1,
  pin: 2121,
};
...
// const calcAverageHumanAge = dogAges1.map((ages, i) => {
//   if (ages <= 2) return const humanAge = 2 *
// })
// 1)
const calcAverageHumanAge = function (ages) {
  const humanAges = ages.map(age => (age <= 2 ? 2 * age : 16 + age * 4));
  // const humanAges = ages.map((age, i) => {
...
// .some basically means "any" because if anything is true, it'll return true
...
// You can choose how many levels deep that flat goes
...
// flatMap (can only go 1 level deep though)
...
// Trying to recreate above function with only flatMap and reduce. revisit this and try again later
const bankDepositSums = accounts
  .flatMap(acc => acc.movements)
  .reduce((sum, cur) => {
    sum + cur, sum > 0 ? sum + cur : sum;
    return sum;
  });
...
let a = 10;
// This does not change the value until you call it again later
console.log(a++);
// This does change the value immediately
console.log(++a);
console.log(a);
...
// 6.
// console.log(dogs.some(dog => (dog.curFood > (dog.recFood * 0.9)) && (current < (dog.recFood * 1.1))))
